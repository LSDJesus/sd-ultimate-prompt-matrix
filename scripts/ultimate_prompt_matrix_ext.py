"""
Ultimate Prompt Matrix Extension v7.3.3 (UI Hotfix) for AUTOMATIC1111 & Forge
...
REX_PARAM = re.compile(r'<(?!random\b)([a-zA-Z_]+)\(([^)]+)\)>') # Fixed: Negative lookahead for 'random'
...
# This function is no longer used with gr.Gallery
# def update_image_display(slider_value, image_list):
#     if not image_list or not slider_value: return None
#     index = int(slider_value) - 1
#     if 0 <= index < len(image_list): return image_list[index]
#     return None
...
def draw_grid_with_annotations(grid_images, x_labels, y_labels, margin_size, title="", show_annotations=True):
    ...
    if num_rows == 0 or num_cols == 0: return None # Fixed: Prevent division by zero
...
def run_matrix_processing(p_text, n_text, seed_behavior, axis_overrides, base_params, margin, mega, desc_fn, save_list, show_anno, dyn_prompts):
    ...
    for i, page_label in enumerate(grid_info['page_labels']):
        ...
        if grid_image:
            ...
            if opts.grid_save: images.save_image(grid_image, (opts.outdir_grids or opts.outdir_txt2img_grids), f"matrix_{sanitize_filename(page_label) if desc_fn else f'page_{i+1}'}", p=p_job)
    ...
    if mega_grid and opts.grid_save: images.save_image(mega_grid, (opts.outdir_grids or opts.outdir_txt2img_grids), "mega-matrix", p=p_job)
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as ui_component:
        ...
        def on_calculate_click(prompt_text, neg_prompt_text, w, h, base_speed, threshold):
            ...
            # Fixed: Parse prompts separately to retain origin
            ...
            x_opts = grid_axes[-1]['token'] if len(grid_axes) > 0 else "N/A" # Fixed: Show full token
            y_opts = grid_axes[-2]['token'] if len(grid_axes) > 1 else "N/A" # Fixed: Show full token
        ...
        def on_generate_click(dry_run, p_text, n_text, detected_axes, *args):
            ...
            # Package all the base UI settings into a dictionary
            ...
            if dry_run:
                # Dry run is not a generator, it just returns a dict
                result = run_matrix_processing(*run_args_list)
                yield {html_log: result.get('html_log', '')}
            else:
                for update in run_matrix_processing(*run_args_list):
                    # Fixed: Yield a dict where keys are components, not strings
